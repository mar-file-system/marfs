...
impl fmt::Display for SubTask {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.pad(&format!("Subtask{} of \"{}\" Task@{:?}", self.count, &self.taskdef.name, DateTime::<Local>::from(self.timestamp)))
    }
}
    ...
    fn drop(&mut self) {
        ...
        if let TaskChild::Child(ref mut child) = self.child {
            match child.kill() {
                ...
                Err(error) => panic!(
                    "FATAL ERROR: {self} failed to send SIGKILL to child process: {error}"
                ),
            }
        }
        // clean up our output file
        let mut ppath = ProcessingPath::new(Some(self.pid), Some(self.timestamp), ProcessingPathElement::SubTaskOutput(self.count));
            ...
            if let Some(rnamedir) = rnamepath.parent() {
                if let Err(error) = fs::create_dir_all(rnamedir) {
                    match error.kind() {
                        ...
                        _ => {
                            eprintln!("ERROR: {self} failed to create failure output dir location \
...
impl SubTask {
    ...
    pub fn get_writer(&self) -> Result<LineWriter<fs::File>,String> {
        ...
        let mut ppath = ProcessingPath::new(Some(self.pid), Some(self.timestamp), ProcessingPathElement::SubTaskDir(self.count));
        let dirpath = PathBuf::from(&ppath);
        match fs::create_dir(&dirpath) {
            Err(e) => match e.kind() {
                ...
                _ => return Err(
                    format!(
                        "failed to create sub-task subdir {dirpath:?}: {e}"
                    )
                ),
            },
            Ok(()) => (),
        ...
        let inputfile = match fs::OpenOptions::new()
                                              ...
                                              .create(true)
                                              .open(&inputpath) {
            Err(error) => return Err(format!("failed to open sub-task input file {inputpath:?}: {error}")),
            Ok(file) => file,
        };
    ...
    pub fn launch(&mut self) -> Result<(),String> {
        ...
        let mut ppath = ProcessingPath::new(Some(self.pid), Some(self.timestamp), ProcessingPathElement::SubTaskInput(self.count));
        if let Some(val) = self.cmdvals.insert(
            String::from("file"),
            String::from(&ppath)
            ) {
            panic!(
                "FATAL ERROR: {self} cmdvals table held pre-existing 'file' target: \"{val}\""
            );
        }
        ...
        let outfile = match fs::OpenOptions::new().write(true).create_new(true).open(&outpath) {
            Err(e) => return Err(
                format!(
        ...
        let child = match command.stdin(Stdio::null()).stdout(outfile).stderr(errfile).spawn() {
    ...
    pub fn poll(&mut self) -> &TaskChild {
        ...
        let child = match self.child {
            ...
            _ => panic!(
                "FATAL ERROR: {self} not yet launched!"
            ),
        };
        ...
        match child.try_wait() {
            ...
            Ok(None) => (),
            Err(e) => eprintln!(
                "could not query child proc status: {e}"
            ),
        }
        if let Some(timeout) = self.taskdef.timeout {
            ...
            let ppath = ProcessingPath::new(Some(self.pid), Some(self.timestamp), ProcessingPathElement::SubTaskOutput(self.count));
            let outpath = PathBuf::from(&ppath);
        ...
        if killproc {
            match child.kill() {
                ...
                Err(error) => panic!(
                    "FATAL ERROR: {selfdisplay} failed to SIGKILL child process: {error}"
                ),
            }
        }
        &self.child
