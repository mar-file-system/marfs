...
static PATH_SEGS: &[&str] = &["pod", "block", "cap", "scat"];
    ...
    match Connection::open_with_flags(
        ...
        Ok(conn) => {
            ...
            let _ = conn.execute_batch(
                "CREATE TABLE IF NOT EXISTS push (name TEXT PRIMARY KEY, mtime int64);",
            );
            let _ = conn.execute_batch("CREATE TABLE IF NOT EXISTS mtime (oldest int64);");
            Ok(conn)
        }
        Err(msg) => Err(format!(
            "Could not open PUSH database {}: {}",
            dbname.display(),
            msg
        )),
    }
...
fn process_leaf(path: PathBuf, fp: Arc<FlushPush>, output: Output) {
    ...
    let util = match get_utilization(&path) {
        ...
        Err(msg) => {
            eprintln!(
                "Warning: Getting utilization for {} failed: {}",
                path.display(),
                msg
            );
            return;
    ...
    let entries = match fs::read_dir(&path) {
        Ok(list) => list,
        Err(msg) => {
            eprintln!("Error: Could not read_dir {}: {}", path.display(), msg);
            return;
        }
    };
    ...
    for entry_res in entries {
        ...
        if let Ok(entry_type) = entry.file_type() {
            ...
            eprintln!(
                "Warning: Could not get type of {}. Skipping.",
                child.display()
            );
            continue;
        }
            ...
            if let Ok(mtime) = st.modified() {
                if let Ok(file_age) = fp.config.file_age(mtime) {
                    ...
                    let flush = fp.ops.flush
                        && (fp.force
                            || ((mtime >= prev_oldest_mtime) && (file_age.as_secs() > max_age)));
                    ...
                    if flush {
                        ...
                        let _ =
                            pushdb.execute("DELETE FROM push WHERE name == ?1", params![basename]);
                    ...
                    if let Ok(_) = pushdb.execute(
                        "INSERT INTO push (name, mtime) VALUES (?1, ?2)",
                        params![basename, msecs],
                    ) {
                    ...
                    match pushdb.query_row::<u64, _, _>(
                        "SELECT mtime FROM push WHERE name == ?1",
                        params![basename],
                        |row| row.get(0),
                    ) {
                        Ok(old_mtime) => {
                            ...
                            if old_mtime < mtime {
                                ...
                                if let Err(msg) = pushdb.execute(
                                    "UPDATE push SET mtime = ?1 WHERE name == ?2",
                                    params![msecs, basename],
                                ) {
    ...
    if (curr_oldest_mtime > prev_oldest_mtime) && (curr_oldest_mtime != future_mtime) {
        let msecs = curr_oldest_mtime
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        let _ = pushdb.execute("INSERT INTO mtime (oldest) VALUES (?1)", params![msecs]);
    }
}
 ...
 */
fn print_flushable_in_dal(dal_root: &PathBuf, fp: FlushPush, output: Output, nthreads: usize) {
    ...
    fn write_paths(
        ...
        pool.execute(move || {
            ...
            let mut output_file = match fs::File::create(output_path.clone()) {
                Ok(file) => file,
                Err(msg) => panic!(
                    "Error: Could not open flush file {}: {}",
                    output_path.display(),
                    msg
                ),
            };
...
struct Cli {
    ...
    #[arg(
        help = "Regex pattern which objects must match in order to be eligible for any operation"
    )]
    must_match: Option<Regex>,
