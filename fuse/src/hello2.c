...
-- the fuse_context available in getattr() is not the same one given to read().
...
#if 1
      static void* private_value = NULL;
      int init = 0;
      struct fuse_context* ctx = fuse_get_context();
      if (! ctx->private_data) {
         init = 1;
         ctx->private_data = ++private_value;
      }
#endif
                      ...
                      struct fuse_file_info *fi)
{
   ...
   int count = snprintf(buf, size,
                        "Hello World, fi=0x%lx, fi->fh=%lx, ctx->private_data=0x%lx %s\n",
                        (ssize_t)fi,
                        (ssize_t)fi->fh,
                        (ssize_t)ctx->private_data,
                        (init ? "(init)" : ""));
   if (count < 0)
      return -EIO;
   return count;
